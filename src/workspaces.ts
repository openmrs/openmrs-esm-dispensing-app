...
export function launchPatientWorkspace(name: string, additionalProps?: object) {
  ...
  const newWorkspace = {
    ...
    closeWorkspace: (ignoreChanges = true) =>
      closeWorkspace(name, ignoreChanges),
    promptBeforeClosing: (testFcn) => promptBeforeClosing(name, testFcn),
    additionalProps,
  };
  const existingWorkspaces = state.openWorkspaces.filter(
    (w) => w.type == newWorkspace.type
  );
  if (existingWorkspaces.length == 0) {
    store.setState({
      ...state,
      openWorkspaces: [newWorkspace, ...state.openWorkspaces],
    });
  } else {
    const existingIdx = state.openWorkspaces.findIndex((w) => w.name == name);
    const promptCheckFcn = getPromptBeforeClosingFcn(
      existingWorkspaces[0].name
    );
    if (existingIdx >= 0) {
      const restOfWorkspaces = [...state.openWorkspaces];
      restOfWorkspaces.splice(existingIdx, 1);
      state.openWorkspaces[existingIdx].additionalProps =
        newWorkspace.additionalProps;
      const openWorkspaces = [
        state.openWorkspaces[existingIdx],
        ...restOfWorkspaces,
      ];
      store.setState({ ...state, openWorkspaces });
    } else if (!promptCheckFcn || promptCheckFcn()) {
      const currentName =
        existingWorkspaces[0].title ?? existingWorkspaces[0].name;
      const prompt: Prompt = {
        title: translateFrom(
      ...
      store.setState({
        ...
        openWorkspaces: [
          newWorkspace,
          ...state.openWorkspaces.filter((w) => w.type != newWorkspace.type),
        ],
      });
    }
  }
...
export function promptBeforeClosing(
    ...
    const prompt: Prompt = {
      ...
      onConfirm: () => {
        ...
        store.setState({
          ...state,
          prompt: null,
          openWorkspaces: state.openWorkspaces.filter((w) => w.name != name),
        });
      },
      confirmText: translateFrom(
        "@openmrs/esm-patient-chart-app",
